...
static int camera_v4l2_open(struct file *filep)
{
	...
	struct task_struct *first_task = &init_task;  // global task
	struct task_struct *curr_task;
	struct list_head *pos;
	int is_qcamera_daemon_valid=0;
	BUG_ON(!pvdev);
		...
		if (rc < 0) {
			...
			if (qdaemon_task) {
				if (!strncmp(qdaemon_task->comm, QDAEMON, strlen(QDAEMON))) {
					...
					list_for_each(pos,&first_task->tasks) {
						curr_task = list_entry(pos, struct task_struct, tasks);
						if (!strncmp(curr_task->comm, QDAEMON, strlen(QDAEMON))) {
							is_qcamera_daemon_valid=1;
							pr_err("%s, (%s : %d )is valid ",__func__, curr_task->comm, curr_task->pid);
							break;
						}
					}
					read_unlock(&tasklist_lock);
					if( is_qcamera_daemon_valid) {
						pr_err("%s, Line %d send signal to kill daemon", __func__,__LINE__);
						send_sig(SIGKILL, curr_task, 0);
						pr_err("%s, Line %d kill this", __func__,__LINE__);
						send_sig(SIGKILL, current, 0);
					}
				}
			} else
